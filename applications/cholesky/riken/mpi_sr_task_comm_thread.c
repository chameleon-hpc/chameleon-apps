...
#pragma omp task untied depend(out:A[k][k]) firstprivate(k)
...
#pragma omp task untied depend(in:A[k][k]) firstprivate(k, dst)
        ...
        if (block_rank[k*nt+k] != mype) {
            ...
            if (recv_flag) {
#pragma omp task untied depend(out:B) firstprivate(k)
        ...
        for (int i = k + 1; i < nt; i++) {
            if (block_rank[k*nt+i] == mype) {
                if (block_rank[k*nt+k] == mype) {
#pragma omp task untied depend(in:A[k][k]) depend(out:A[k][i]) firstprivate(k, i) 
                    do_trsm(A[k][k], A[k][i], ts, ts);
                } else {
#pragma omp task untied depend(in:B) depend(out:A[k][i]) firstprivate(k, i) 
            ...
            if (block_rank[k*nt+i] == mype && np != 1) {
                ...
                for (int dst = 0; dst < np; dst++) {
                    if (send_flags[dst] && dst != mype) {
#pragma omp task untied depend(in:A[k][i]) firstprivate(k, i, dst) 
            ...
            if (block_rank[k*nt+i] != mype) {
                ...
                if (recv_flag) {
#pragma omp task untied depend(out:C[i]) firstprivate(k, i)
...
#pragma omp task untied depend(in:A[k][i], A[k][j]) depend(out:A[j][i]) firstprivate(k, j, i) 
                        do_gemm(A[k][i], A[k][j], A[j][i], ts, ts);
                    } else if (block_rank[k*nt+i] != mype && block_rank[k*nt+j] == mype) {
#pragma omp task untied depend(in:C[i], A[k][j]) depend(out:A[j][i]) firstprivate(k, j, i) 
                        do_gemm(C[i], A[k][j], A[j][i], ts, ts);
                    } else if (block_rank[k*nt+i] == mype && block_rank[k*nt+j] != mype) {
#pragma omp task untied depend(in:A[k][i], C[j]) depend(out:A[j][i]) firstprivate(k, j, i) 
                        do_gemm(A[k][i], C[j], A[j][i], ts, ts);
                    } else {
#pragma omp task untied depend(in:C[i], C[j]) depend(out:A[j][i]) firstprivate(k, j, i) 
            ...
            if (block_rank[i*nt+i] == mype) {
                if (block_rank[k*nt+i] == mype) {
#pragma omp task untied depend(in:A[k][i]) depend(out:A[i][i]) firstprivate(k, i) 
                    do_syrk(A[k][i], A[i][i], ts, ts);
                } else {
#pragma omp task untied depend(in:C[i]) depend(out:A[i][i]) firstprivate(k, i) 
