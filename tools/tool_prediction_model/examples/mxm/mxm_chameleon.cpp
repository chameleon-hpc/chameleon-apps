...
void printHelpMessage() {
    std::cout<<"Usage: mpiexec -n np ./matrixExample nt_0 nt1 nt_..." << std::endl;
...
int main(int argc, char **argv)
{
    ...
    const int num_ranks = iNumProcs;
    int total_load_per_rank[num_ranks];
    for (int i = 0; i < iNumProcs; i++)
        total_load_per_rank[i] = 0;
    ...
    // necessary to be aware of binary base addresses
    // to calculate offset for target entry functions
	...
	for (int i = 0; i < numberOfTasks; i++) {
        ...
        matrixSize[i] = (rand() % 2048) + 512;    // just for testing the tool

        total_load_per_rank[iMyRank] += matrixSize[i];
    ...
    printf("Total load per R%d: %d\n", iMyRank, total_load_per_rank[iMyRank]);
