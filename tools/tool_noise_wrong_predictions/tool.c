    ...
    int32_t num_tasks_stolen_rep)
{
   ...
   if (rank%2==1){
     return 0.1*(num_tasks_local + num_tasks_local_rep + num_tasks_stolen);
   }
   else {
...
)
{
    ...
    int rank = cham_t_get_rank_info()->comm_rank;
    if (rank%2==1 && tasks_processed_sync_cycle >= start_tasks_processed_per_rank && num_noise_generated<start_tasks_processed_per_rank){ // && load_info_per_rank != 0){
        //printf("R%d, start=%d tasks_id=%d, cur_load = %d / total_created_tasks = %d\n", rank, start_tasks_processed_per_rank, chameleon_get_task_id(task), load_info_per_rank, total_created_tasks_per_rank);
        //noise_time = (int) ((double) noise*((1.0*total_created_tasks_per_rank-load_info_per_rank)/start_tasks_processed_per_rank)); // make noise 50% slower for a mxm-task with size = 1024
        noise_time = noise;
        // noise_time = 507850;    // make noise 50% slower for non-uni tasks with size_range = {128, 256, 512, 1024, 2048}
        //printf("noise %d\n", noise_time);
        acc_noise += noise_time;
        num_noise_generated++;
    }
