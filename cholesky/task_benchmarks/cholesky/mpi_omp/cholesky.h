...
static void convert_to_blocks_with_rank(const int ts, const int DIM, const int N, double Alin[N][N], double *A[DIM][DIM], int *block_rank, int rank)
{
    for (int i = 0; i < DIM; i++)
        for (int j = 0; j < DIM; j++) {
            if (block_rank[i*DIM + j] == rank) {
                gather_block ( N, ts, &Alin[i*ts][j*ts], A[i][j]);
            }
        }
}
