...
#pragma omp parallel private(k, xs, xe, ys, ye) shared(u, uu)
{
#pragma omp single
    for (k = 0; k < niter; k++) {
                 ...
                 firstprivate(by) 
{
        ...
        for (bx = 0; bx < max_bx; bx++) {
#pragma omp task depend(out:uu[bx][max_by-1]) \
                 firstprivate(bx) 
{
            if (right != MPI_PROC_NULL) {
...
#pragma omp task depend(out:uu[bx][0]) \
                 firstprivate(bx) 
{
            if (left != MPI_PROC_NULL) {
        ...
        for (by = 0; by < max_by; by++) {
#pragma omp task depend(out:uu[max_bx-1][by]) \
                 firstprivate(by) 
{
            if (lower != MPI_PROC_NULL) {
...
#pragma omp task depend(out:uu[0][by]) \
                 firstprivate(by) 
{
            if (upper != MPI_PROC_NULL) {
        ...
        for (bx = 0; bx < max_bx; bx++) {
            for (by = 0; by < max_by; by++) {
                ...
                yl = (by == 0) ? 0 : 1;
                yu = (by == max_by - 1) ? 0 : 1;
#pragma omp task depend(in:uu[bx-xl][by], \
                           uu[bx+xu][by], \
                           uu[bx][by-yl], \
                           uu[bx][by+yu], \
                           uu[bx][by]) \
